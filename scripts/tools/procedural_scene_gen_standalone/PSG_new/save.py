...
from scripts.procedural_scene_gen_standalone.PSG_new.utils import compute_node_depths
...
class SaveManager:
    ...
    def save_manager(self, scenes, graph):
        """
        Main saving function that decides which saving functions to call based on configuration.

        Args:
            scenes (list): List of generated scenes to save
            graph (dict): Parent-child graph used for scene generation

        Returns:
            int: Status code (0 for success)
        """
        # Convert dictionary graph to NetworkX directed graph for visualization
        nx_graph = nx.DiGraph(graph)

        # Process each scene based on configuration
        for i, scene in enumerate(scenes):
            # Save positional information as JSON if enabled
            if self.config["output"]["save"].get("positional_info", False):
                self.save_positional_info(scene, i)

            # Save USD file if enabled
            if self.config["output"]["save"].get("usd", False):
                self.save_usd(scene, i)

        # Save parent-child graph visualization if enabled
        if self.config["output"]["save"].get("parent_child_graph", False):
            self.save_parent_child_graph(nx_graph)
    ...
    def save_parent_child_graph(self, graph, filename="parent_child_graph.png"):
        ...
        node_colors = [pastel_colors[depths[n]] for n in graph.nodes()]
        ...
        legend_handles = [Patch(color=pastel_colors[i], label=f"Depth {i}") for i in range(max_depth + 1)]
        ax.legend(
            handles=legend_handles, loc="upper center", bbox_to_anchor=(0.5, -0.05), ncol=max_depth + 1, frameon=False
        )

        plt.title("Parent-Child Relationship Graph (by Depth)", fontsize=14)
        plt.axis("off")
